# 農園view (GitHub Pages版) 開発ガイドライン

あなたは、静的サイトとして動作する農園viewの開発エキスパートです。以下のファイル構造と実装方針に従ってコードを生成してください。
...
- `data/farms/<field_id>.json`, `data/farms/<field_id>_finished.json`
    ...
    - `x`: 畝の西端（または左端）のx座標 (number, mm単位。左上が原点)
    - `y`: 畝の北端（または上端）のy座標 (number, mm単位。左上が原点)
    - `direction`: 畝の向き (`"horizontal"` = 東西方向 / `"vertical"` = 南北方向)
    - `row_length`: 畝の長さ (number, mm単位)。`direction`が`"horizontal"`なら東西長さ、`"vertical"`なら南北長さ。
    - `row_width`: 畝の幅 (number, mm単位)。`direction`が`"horizontal"`なら南北幅、`"vertical"`なら東西幅。
...
- **可視化ロジック**:
    ...
    - 実際の畝の形や配置を反映したレイアウト（地図のように）を作成し、視覚的にわかりやすくする。
        ...
        - 農場マップは `position: relative` のコンテナとし、各畝は `position: absolute` で `x, y` 座標（mm単位）をピクセル変換して配置する。
        - マップのサイズは全畝の座標＋長さの最大値から自動算出する。
        - 畝の幅（`row_width`、mm単位）もスケール変換してピクセル高さ（横長畝）または幅（縦長畝）に反映する。
        - 農道（`status: "path"`）は畝と同じスキーマで管理し、1エントリで任意の長さを持てる。縦断・横断いずれも1エントリで表現できる。
        - 状態が "empty" の場合は、畝を点線で表示し、作物名はJSON上設定がない（空文字）であれば、「-」と表示する。
        - 畝上に表示する情報は、作物名のみとし、詳細は選択時のモーダルで表示する。
